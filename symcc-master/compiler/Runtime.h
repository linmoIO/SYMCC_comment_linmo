...
struct Runtime {
  ...
  std::array<SymFnT, llvm::CmpInst::BAD_ICMP_PREDICATE> // 将比较操作映射到符号表达式
      comparisonHandlers{};

  /// Mapping from binary operators to the functions that build the
  /// corresponding symbolic expressions.
  std::array<SymFnT, llvm::Instruction::BinaryOpsEnd> // 将二进制操作映射到符号表达式
      binaryOperatorHandlers{};
};

bool isInterceptedFunction(const llvm::Function &f);  // 判断是否为截取函数
